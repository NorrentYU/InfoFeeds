# Substack Module Execution Pack

## 1. 模块目标
在最近 24h 时间窗口内，基于 Substack RSS（`/feed`）抓取更新，提取可读正文并输出统一中间结构；通过测试后进入 YouTube 模块。

## 2. 模块范围
1. 本轮只做 Substack 渠道，不触发 YouTube/X/Others 开发。
2. 固定测试源：
...
## 3. 开发单（给开发 agent）
开发 agent 执行文件：`contracts/tasks/substack-dev-task.md`

### 3.1 输入
1. `sourceList.md` 的 Substack 分组。
2. 调度参数（默认 24h，UTC+8 08:30）。
3. 用户摘要 prompt（由设置层注入）。

### 3.2 实现任务
1. 将 Substack 源标准化为 feed URL（`/feed`）。
...
### 3.4 交付物
1. Substack 渠道标准化数据样例（至少覆盖两个测试源）。
2. 开发自测记录（含至少 1 次失败重试）。
3. 已知限制列表（若有）。

## 4. 测试单（给测试 agent）
测试 agent 执行文件：`contracts/tasks/substack-test-task.md`
...
1. 开发交付物齐全且符合契约。
2. 测试报告通过且证据完整。
3. Substack 结果可被下游摘要与聚合层直接消费。
4. 仅在上述 1-3 全部满足时，允许切换到 YouTube 模块。
