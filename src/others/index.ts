...
import { validateOthersContent } from "./filters.js";
...
function sourceNameOf(source: OthersSource | string): string {
...
function sourceUrlOf(source: OthersSource | string): string {
  if (typeof source === "string") {
    return source;
  }
  return source.url;
...
export async function fetchOthersSources(
  sources: Array<OthersSource | string>,
  options: OthersFetchOptions = {},
): Promise<OthersFetchResult> {
  const now = options.now ?? new Date();
  const windowHours = options.windowHours ?? 24;
  const latestCountPerSource = options.latestCountPerSource;
  const useLatestMode =
    typeof latestCountPerSource === "number" && latestCountPerSource > 0;
  ...
  for (const source of sources) {
    ...
    if (parsed.entries.length === 0) {
      failures.push(
        failure({
          ...
          detail: useLatestMode
            ? "feed 无可处理条目"
            : `窗口 ${windowHours}h 内无可处理条目`,
          retryable: false,
          attempt: 1,
        }),
      );
    ...
    for (const entry of parsed.entries) {
      ...
      const validation = validateOthersContent({ text: entry.content });
      if (!validation.valid) {
        failures.push(
          failure({
            sourceName,
      ...
      if (useLatestMode && sourceRecordCount >= (latestCountPerSource || 0)) {
        break;
      }
    }

    if (!useLatestMode && inWindowCount === 0) {
      failures.push(
        failure({
          sourceName,
          sourceUrl,
    ...
    if (useLatestMode && sourceRecordCount < (latestCountPerSource || 0)) {
      failures.push(
        failure({
          ...
          feedUrl,
          failureType: "no_updates",
          detail: `仅提取到 ${sourceRecordCount} 篇，低于目标 ${latestCountPerSource} 篇`,
          retryable: false,
          attempt: 1,
        }),
      );
